...
import mapAlphaToBraille, mapBrailleToAlpha, mapAlphaToSignal
...
def char_to_braille(char):
    ...
    if is_braille(char):
        ...
        if open_quotes:
            ...
            #return mapAlphaToBraille.punctuation.get("”")
            return mapAlphaToSignal.punctuation.get("”")
    elif char in mapAlphaToBraille.abbreviation:
        return mapAlphaToBraille.abbreviation.get(char)
...
def word_to_braille(word):
    ...
    ##########################################################
    abbreviation = {abr:word.find(abr) for abr in ABBREVIATION if abr in word.lower()}
    ...
    for abr, pos in abbreviation:
        word[pos:pos+len(abr)] = [''.join(word[pos:pos+len(abr)])]
    ##########################################################
    result = ""
    for char in word:
